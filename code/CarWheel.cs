...
struct ToggWheel
{
	public static Logger Logger = new( "Togg Sedan Wheel" );
	...
	{
		var position = parent.WorldPosition;
		var rotation = parent.WorldRotation;

		var wheelAttachPos = position + offset;
		var wheelExtend = wheelAttachPos - rotation.Up * (length * parent.WorldScale);

		var tr = Game.ActiveScene.Trace.Ray( wheelAttachPos, wheelExtend )
			.IgnoreGameObject( parent.GameObject )
			.IgnoreGameObject( parent.Driver )
			.Run();

		wheel = length * tr.Fraction;
		var wheelRadius = (14 * parent.WorldScale);
		...
		var body = parent.SelfBody.PhysicsBody.SelfOrParent;

		_previousLength = _currentLength;
		_currentLength = (length * parent.WorldScale.x) - tr.Distance;
